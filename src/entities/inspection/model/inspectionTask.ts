...
export type InspectionReadingValues = Partial<Record<InspectionReadingKey, number>>
...
export function getReadingViolations(readings: InspectionReadingValues) {
  return inspectionReadingConfigs.reduce<InspectionReadingViolation[]>(
    (violations, config) => {
      ...
      if (
        typeof config.range.min === 'number' &&
      ...
      if (
        typeof config.range.max === 'number' &&
